...
for service in $ROOT_PATH/deployment/k8s/*/; do
# if new version exists, use it instead of old
  if [ -f "$ROOT_PATH/deployment/k8s/$(basename $service)/kustomization.yaml" ]; then
      echo "---" # yaml separator in stdout
      echo "# NEW VERSION ./kustomization.yaml"
      ${KUSTOMIZE} "$ROOT_PATH/deployment/k8s/$(basename $service)"
      if [ $? -ne 0 ]; then
          echo "[ERROR] $service"
          exit
      fi
